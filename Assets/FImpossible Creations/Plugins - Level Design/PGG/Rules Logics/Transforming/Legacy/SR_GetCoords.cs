...
namespace FIMSpace.Generating.Rules.QuickSolutions
{
    public class SR_GetCoords : SpawnRuleBase, ISpawnProcedureType
    {
        ...
        public override void CellInfluence(FieldSetup preset, FieldModification mod, FieldCell cell, ref SpawnData spawn, FGenGraph<FieldCell, FGenPoint> grid, Vector3? restrictDirection = null)
        {
            ...
            for (int s = 0; s < OwnerSpawner.Rules.Count; s++) { if (OwnerSpawner.Rules[s] is Count.SR_LimitSpawnCount) { spawnCountRule = OwnerSpawner.Rules[s] as Count.SR_LimitSpawnCount; break; } }

            Count.SR_SpawningPropability spawnPropabilityRule = null;
            for (int s = 0; s < OwnerSpawner.Rules.Count; s++) { if (OwnerSpawner.Rules[s] is Count.SR_SpawningPropability) { spawnPropabilityRule = OwnerSpawner.Rules[s] as Count.SR_SpawningPropability; break; } }
            /* Hard Coded Part End */
